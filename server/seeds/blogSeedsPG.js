import { connectDB, getPool } from '../db/db.js';
import fs from 'fs';
...
const dubaiBlogs = [
  {
    ...
    author_bio: "HR Director with 12+ years experience in Dubai's recruitment industry",
    category: "Dubai Jobs",
  ...
  },
  {
    ...
    author_bio: "Recruitment consultant specializing in executive placements in the Gulf region",
  ...
  },
  {
    ...
    author_bio: "Senior HR Manager at a leading Dubai-based multinational corporation",
  ...
  },
  {
    ...
    author_bio: "Senior Software Engineer at a Dubai-based fintech startup",
    category: "Dubai Jobs",
  ...
  },
  {
    ...
    author_bio: "Executive recruiter with 15+ years experience placing candidates in Dubai's top companies",
...
async function seedBlogs() {
  try {
    ...
    const schemaPath = join(__dirname, '..', 'db', 'blogSchema.sql');
    const schema = fs.readFileSync(schemaPath, 'utf8');
    await pool.query(schema);
    console.log('✅ Blog schema created/updated');
    ...
    for (const blog of dubaiBlogs) {
      const slug = generateSlug(blog.title);
      
      const insertQuery = `
        INSERT INTO blog_posts (
          title, slug, excerpt, content, author_name, author_bio, 
          category, tags, featured, read_time
    ...
    console.log(`🎉 Successfully inserted ${dubaiBlogs.length} blog posts!`);
    process.exit(0);
  } catch (error) {
