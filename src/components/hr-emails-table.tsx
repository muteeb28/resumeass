    ...
    <>
      {Array.from({ length: 7 }).map((_, i) => (
        <tr key={i} className="border-b border-hub-border animate-pulse">
          ...
          <td className="px-4 py-[11px]"><div className="h-[13px] w-24 bg-hub-bg-subtle rounded" /></td>
          <td className="px-4 py-[11px]"><div className="h-[13px] w-36 bg-hub-bg-subtle rounded" /></td>
          <td className="px-4 py-[11px]"><div className="h-[13px] w-20 bg-hub-bg-subtle rounded" /></td>
          <td className="px-4 py-[11px]"><div className="h-[13px] w-10 bg-hub-bg-subtle rounded" /></td>
          <td className="px-4 py-[11px]"><div className="h-[19px] w-14 bg-hub-bg-subtle rounded-[4px]" /></td>
        </tr>
      ))}
    </>
  );
...
function LinkIcon({ href, icon: Icon, label }: { href: string; icon: React.ElementType; label: string }) {
  ...
  return (
    <a
      ...
      aria-label={label}
      className="p-1 rounded-[4px] text-hub-text-3 outline-none transition-colors hover:text-hub-text-1 hover:bg-hub-bg-subtle"
    >
    ...
    <div className={cn("bg-hub-surface border border-hub-border rounded-[14px] overflow-hidden flex flex-col relative", className)}>
      ...
      {hasAccess && (
        <div className="p-3 border-b border-hub-border bg-hub-bg-subtle/30">
          <form onSubmit={handleSubmit} className="relative max-w-sm flex gap-2">
            ...
            <button type="submit" className="px-3 py-1.5 bg-hub-text-1 text-hub-surface rounded-md text-[12.5px] font-medium hover:opacity-90 transition-opacity">
              Search
            </button>
          </form>
        </div>
      ...
      <div className={cn("overflow-x-auto overflow-y-auto flex-grow relative", tableClassName)}>
                  ...
                  >
                    ...
                    <td className="px-4 py-[11px] min-w-[220px]">
                      <div className="flex items-center gap-2 group/email">
                        <span className={cn("text-[12.5px] truncate max-w-[160px]", !isDemoRow && !isRevealed ? "text-hub-text-3 tracking-widest font-mono select-none" : "text-hub-text-2")}>
                          {displayEmail || "—"}
                        </span>
        ...
        {!hasAccess && !loading && (
          <div className="absolute left-0 right-0 bottom-0 top-auto h-[45%] z-30 flex items-end justify-center bg-gradient-to-t from-hub-surface via-hub-surface/90 to-transparent p-4 pb-6 pointer-events-none">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.05 }} className="w-full max-w-xl rounded-xl border border-slate-200/90 bg-white p-4 shadow-[0_16px_48px_rgba(15,23,42,0.1)] backdrop-blur-sm sm:p-5 pointer-events-auto">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3.5">
                  ...
                  <div className="max-w-xs sm:max-w-sm">
                    ...
                    <p className="mt-0.5 text-xs font-semibold text-slate-900">{userLoggedIn ? "Unlock remaining HR contacts" : "Sign in to access more contacts"}</p>
                    <p className="mt-0.5 text-[11px] leading-4 text-slate-500">You are previewing live items. Active tier removes the bottom fade constraint instantly.</p>
                  </div>
                </div>
                <div className="flex flex-col gap-2 shrink-0 sm:items-end">
                  {!userLoggedIn ? (
                    <a href={loginHref} className="inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-lg bg-slate-900 px-3.5 py-2 text-xs font-semibold text-white shadow-sm hover:bg-slate-800 transition-colors">
                      Login <ArrowRight className="h-3 w-3" />
                    </a>
                  ) : (
                    <a href={membershipHref} className="inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-lg bg-amber-500 px-3.5 py-2 text-xs font-semibold text-slate-950 shadow-sm hover:bg-amber-400 transition-colors">
      ...
      {hasAccess && !loading && contacts.length > 0 && (
        <div className="px-4 py-3 border-t border-hub-border flex items-center justify-between bg-hub-bg-subtle/10">
          <span className="text-[12px] text-hub-text-3">Page <strong>{page}</strong> of <strong>{totalPages}</strong></span>
          <div className="flex items-center gap-2">
            <button disabled={page === 1} onClick={() => onPageChange(page - 1)} className="p-1.5 rounded-md border border-hub-border bg-hub-surface disabled:opacity-40 hover:bg-hub-bg-subtle transition-colors">
              <ChevronLeft size={14} />
            </button>
            <button disabled={page >= totalPages} onClick={() => onPageChange(page + 1)} className="p-1.5 rounded-md border border-hub-border bg-hub-surface disabled:opacity-40 hover:bg-hub-bg-subtle transition-colors">
