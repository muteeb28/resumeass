  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      if (!membership || membership.status !== "active") {
        // Fallback for unauthorized/anonymous users
        url = "/hr/list/demo";
      } else {
        // Premium or Ultra tiers matching the explicit route logic for India
        const shouldCount = page === 1 ? "&count=true" : "";
        url = `/hr/list/purchased/india?page=${page}&company=${searchTerm}${shouldCount}`;
      }
